...
def load_job_config(path: Union[str, Path]) -> JobConfig:
    ...
    if "params" in data and data["params"] is not None:
        ...
        # Resolve editor-package media paths relative to the job file, just
        # like top-level video/BGM paths. Keep this narrow so arbitrary
        # parameter strings are never treated as filesystem paths.
        for key in ("bgm_ambient_path", "original_audio_path"):
            value = data["params"].get(key)
            if isinstance(value, str) and value and not Path(value).is_absolute():
                data["params"][key] = str((config_path.parent / value).resolve())
