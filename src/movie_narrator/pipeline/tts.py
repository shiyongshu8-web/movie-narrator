...
def generate_voice(ctx: Context) -> Context:
    ...
    if settings.tts_provider is TTSProviderType.ZHIPU:
        # Speed, volume, and watermark settings change generated audio.
        style_prompt = (
            f"{style_prompt}|zhipu_speed={settings.zhipu_tts_speed}"
            f"|zhipu_volume={settings.zhipu_tts_volume}"
            f"|zhipu_watermark={settings.zhipu_tts_watermark_enabled}"
        )
    elif settings.tts_provider is TTSProviderType.MINIMAX:
        # MiniMax request controls affect the returned audio and therefore
        # must participate in the cache identity.  The API key is deliberately
        # excluded so rotating credentials does not duplicate identical audio.
        style_prompt = (
            f"{style_prompt}|minimax_speed={settings.minimax_tts_speed}"
            f"|minimax_volume={settings.minimax_tts_volume}"
            f"|minimax_pitch={settings.minimax_tts_pitch}"
            f"|minimax_emotion={settings.minimax_tts_emotion or ''}"
            f"|minimax_sample_rate={settings.minimax_tts_sample_rate}"
            f"|minimax_bitrate={settings.minimax_tts_bitrate}"
            f"|minimax_format={settings.minimax_tts_format}"
            f"|minimax_channel={settings.minimax_tts_channel}"
            f"|minimax_watermark={settings.minimax_tts_aigc_watermark}"
        )
    ...
    def _key(seg_text: str) -> TTSCacheKey:
        return TTSCacheKey(
            ...
            model=(
                ...
                else settings.zhipu_tts_model
                if settings.tts_provider is TTSProviderType.ZHIPU
                else settings.minimax_tts_model
                if settings.tts_provider is TTSProviderType.MINIMAX
                else ""
            ),
            voice=voice,
            text=seg_text,
