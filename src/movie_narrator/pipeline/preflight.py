...
- TTS check: provider-dependent.
  ...
  - openai / mimo / zhipu: verify ``ConfigError`` is not raised during provider
...
from ..utils.llm import get_llm_client, get_llm_request_kwargs
    ...
    try:
        with get_llm_client() as llm:
            llm.client.chat.completions.create(
                ...
                max_tokens=1,
                **get_llm_request_kwargs(settings),
            )
...
def _check_tts(ctx: Context) -> None:
    ...
    For edge TTS, no probe is needed.  For openai / mimo / zhipu / minimax, provider
    ...
    # For openai / mimo / zhipu / minimax, instantiate the provider to catch ConfigError
