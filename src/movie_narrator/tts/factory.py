...
Built-in providers (edge, openai, mimo, zhipu, minimax) are registered at import
...
def _make_zhipu(settings: Settings) -> TTSProvider:
    from .zhipu_provider import ZhipuTTSProvider

    return ZhipuTTSProvider(settings)


def _make_minimax(settings: Settings) -> TTSProvider:
    from .minimax_provider import MiniMaxTTSProvider
...
for _name, _factory in [
    ...
    (TTSProviderType.ZHIPU.value, _make_zhipu),
    (TTSProviderType.MINIMAX.value, _make_minimax),
]:
