...
# TTS cost per 1K characters. Zhipu pricing is account/model dependent and is
# not assumed here.
_TTS_COST_PER_1K_CHARS: Dict[str, float] = {
    ...
    "mimo": 0.0,
    "zhipu": 0.0,
    "minimax": 0.0,  # account/model pricing varies; keep estimate explicit
}
...
class TTSCallRecord:
    ...
    provider: str  # "edge" | "openai" | "mimo" | "zhipu" | "minimax"
    model: str = ""
    characters: int = 0
    segments: int = 1
    cached: bool = False
...
class CostTracker:
    ...
    def record_tts_call(
        ...
        ``provider`` should be one of the registered TTS providers. ``cached=True`` marks the segment as served from
