...
def _print_first_run_notice(env_path: Path) -> None:
    ...
    print(
        ...
        f"    MN_DEFAULT_VOICE — TTS 语音 (如 zh-CN-YunxiNeural、tongtong 或 MiniMax voice_id)\n"
        f"  配置完成后重新运行即可。\n"
        f"\n"
        f"  ⚠ 合规提示：默认 TTS 通道 edge 基于逆向非官方接口，仅供个人/非商用\n"
        f"    免费测试；正式使用可将 MN_TTS_PROVIDER 切换为 openai、mimo、zhipu 或 minimax。\n",
        file=sys.stderr,
    )
...
class Settings(BaseSettings):
    ...
    # Optional JSON object forwarded unchanged to OpenAI-compatible chat
    # completion requests. Empty/unset means no extra request body fields.
    ...
    zhipu_tts_model: str = "glm-tts"
    zhipu_tts_api_key: Optional[str] = None
    zhipu_tts_base_url: str = "https://open.bigmodel.cn/api/paas/v4"
    zhipu_tts_speed: float = 1.0
    zhipu_tts_volume: float = 1.0
    zhipu_tts_watermark_enabled: bool = True
    zhipu_tts_timeout: int = 60
    minimax_tts_model: str = "speech-2.8-hd"
    minimax_api_key: Optional[str] = None
    minimax_base_url: str = "https://api.minimaxi.com/v1"
    minimax_tts_speed: float = 1.0
    minimax_tts_volume: float = 1.0
    minimax_tts_pitch: int = 0
    minimax_tts_emotion: Optional[str] = None
    minimax_tts_sample_rate: int = 32000
    minimax_tts_bitrate: int = 128000
    minimax_tts_format: str = "mp3"
    minimax_tts_channel: int = 1
    minimax_tts_aigc_watermark: bool = False
