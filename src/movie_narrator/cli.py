...
def _configure_cli_stream_errors() -> None:
    """Prevent legacy Windows code pages from crashing Rich help output."""
...
@app.command("cinematic-lock")
def cinematic_lock(
    matches: str = typer.Option(..., "--matches", help="候选 matches.json"),
    selections: str = typer.Option(..., "--selections", help="人工选择 JSON"),
    scene_database: str = typer.Option(
        ..., "--scene-database", help="对应的 scene_database.json"
    ),
    output: Optional[str] = typer.Option(
        None, "--output", help="锁定结果路径（默认 matches.locked.json）"
    ),
    allow_partial: bool = typer.Option(
        False, "--allow-partial", help="允许仍有 CANDIDATE 的部分锁定结果"
    ),
):
    """Validate and lock human scene selections without mutating candidates."""
    from .cinematic.review import lock_matches

    matches_path = Path(matches)
    target = Path(output) if output else matches_path.with_name("matches.locked.json")
    try:
        locked, manifest = lock_matches(
            matches_path=matches_path,
            selections_path=selections,
            scene_database_path=scene_database,
            output_path=target,
            require_all=not allow_partial,
        )
    except Exception as exc:  # noqa: BLE001 — CLI top-level error barrier
        typer.echo(f"cinematic lock failed: {exc}", err=True)
        raise typer.Exit(code=1)
    typer.echo(f"locked matches: {locked}")
    typer.echo(f"review manifest: {manifest}")
...
def create(
    ...
    mode: str = typer.Option(
        "classic",
        "--mode",
        help="执行引擎 classic|cinematic / Execution engine (default: classic)",
    ),
    cinematic_asr: str = typer.Option(
        "auto",
        "--cinematic-asr",
        help="cinematic ASR: auto|none|whisperx|faster-whisper|funasr",
    ),
    cinematic_visual_analysis: bool = typer.Option(
        True,
        "--cinematic-visual-analysis/--no-cinematic-visual-analysis",
        help="启用 cinematic 视觉分析 / Enable cinematic visual analysis",
    ),
    cinematic_top_k: int = typer.Option(
        5,
        "--cinematic-top-k",
        min=1,
        max=50,
        help="每段旁白保留的候选镜头数 / Cinematic scene candidate count",
    ),
    cinematic_resume: bool = typer.Option(
        False,
        "--cinematic-resume",
        help="校验源文件、参数和产物哈希后恢复 cinematic 运行",
    ),
    cinematic_locked_matches: Optional[str] = typer.Option(
        None,
        "--cinematic-locked-matches",
        help="经 mn cinematic-lock 核验的 matches.locked.json",
    ),
    cinematic_visual_embedding_model: Optional[str] = typer.Option(
        None,
        "--cinematic-visual-embedding-model",
        help="显式启用跨模态镜头检索，例如 clip-ViT-B-32",
    ),
    ...
    edit_package_export: bool = typer.Option(
        False,
        "--export-edit-package",
        "--export-stems",
        help="导出 WAV stems 与多轨时间线 / Export WAV stems and a multi-track timeline",
    ),
    original_audio_path: Optional[str] = typer.Option(
        None,
        "--original-audio",
        help="单独提供原片音频文件 / Optional source-audio file for the editable package",
    ),
    original_audio_verified: bool = typer.Option(
        False,
        "--original-audio-verified",
        help="标记提供的原片音频已核验 / Mark supplied source audio as verified",
    ),
    ...
    if normalized_mode not in {"classic", "cinematic"}:
        raise typer.BadParameter(
            "mode must be classic or cinematic",
            param_hint="--mode",
        )
    if normalized_mode == "cinematic":
        if cinematic_asr not in {
            "auto",
            "none",
            "whisperx",
            "faster-whisper",
            ...
            raise typer.BadParameter(
                "cinematic ASR must be auto, none, whisperx, faster-whisper, or funasr",
                param_hint="--cinematic-asr",
            )
        unsupported = []
        if config:
            unsupported.append("--config")
        if library_dir:
            unsupported.append("--library-dir")
        if research is not None:
            unsupported.append("--research/--no-research")
        if video_format != "16:9":
            unsupported.append("--video-format")
        if keep_cache:
            unsupported.append("--keep-cache")
        if no_clips:
            unsupported.append("--no-clips")
        if strict:
            unsupported.append("--strict")
        if retry:
            unsupported.append("--retry")
        if subtitle_lang:
            unsupported.append("--subtitle-lang")
        if subtitle_mode:
            unsupported.append("--subtitle-mode")
        if narration_preset:
            unsupported.append("--narration-preset")
        if narrator_perspective:
            unsupported.append("--narrator-perspective")
        if focus_character:
            unsupported.append("--focus-character")
        if pause_at:
            unsupported.append("--pause-at")
        if verbose or log_level != "DEBUG":
            unsupported.append("--verbose/--log-level")
        if unsupported:
            raise typer.BadParameter(
                "cinematic engine does not support these options yet: "
                + ", ".join(unsupported),
                param_hint="--mode cinematic",
            )
        if not video:
            raise typer.BadParameter(
                "cinematic mode requires --video movie.mp4",
                param_hint="--video",
            )
        ...
        cinematic_output = (
            ...
            / f"{_sanitize_filename(movie or source_video.stem)}_cinematic"
        )
        from .cinematic.cli_runtime import run_cinematic_create

        try:
            result = run_cinematic_create(
                source_video=str(source_video.resolve()),
                output_dir=cinematic_output,
                style=style,
                target_duration=duration,
                voice=voice,
                bgm_asset=None if no_bgm else bgm,
                asr_backend=cinematic_asr,
                visual_analysis=cinematic_visual_analysis,
                top_k=cinematic_top_k,
                resume=cinematic_resume,
                locked_matches_path=cinematic_locked_matches,
                visual_embedding_model=cinematic_visual_embedding_model,
            )
        except Exception as exc:  # noqa: BLE001 — CLI top-level error barrier
            typer.echo(f"cinematic pipeline failed: {exc}", err=True)
            raise typer.Exit(code=1)
        typer.echo(f"cinematic quality: {result.quality_status}")
        typer.echo(f"scene database: {result.scene_database}")
        typer.echo(f"matches: {result.matches}")
        typer.echo(f"timeline: {result.timeline}")
        typer.echo(f"audio mix: {result.audio_mix}")
        typer.echo(f"quality report: {result.quality_report}")
        typer.echo(str(result.final_video))
        return
    ...
    # These delivery options are intentionally opt-in CLI overrides. YAML
    # values remain available through ``params`` and are not overwritten by
    # the false Typer default.
    resolved_params = dict(resolved.params or {})
    if edit_package_export:
        resolved_params["edit_package_export"] = True
    if original_audio_path:
        original_audio = Path(original_audio_path)
        if not original_audio.is_file():
            raise typer.BadParameter(
                f"original audio not found: {original_audio_path}",
                param_hint="--original-audio",
            )
        resolved_params["original_audio_path"] = str(original_audio.resolve())
    if original_audio_verified:
        resolved_params["original_audio_verified"] = True
...
# ── Source-first visual-event / ChatCut integration commands ───────────────

chatcut_app = typer.Typer(
    help="Official ChatCut MCP connection and editable-timeline operations.",
    no_args_is_help=True,
)
sync_app = typer.Typer(
    help="Build visual-event, narration-binding, and synchronization artifacts.",
    no_args_is_help=True,
)
...
@chatcut_app.command("status")
def chatcut_status():
    """Check the official ChatCut MCP server without editing a project."""

    from .integrations.chatcut.client import probe_chatcut_connection

    report = probe_chatcut_connection()
    typer.echo(report.as_text())
    if not report.ready_for_edit:
        raise typer.Exit(code=2)


@sync_app.command("build")
def sync_build(
    scene_database: str = typer.Option(
        ..., "--scene-database", help="Existing source-first scene_database.json"
    ),
    narration: str = typer.Option(
        ..., "--narration", help="Event-bound NARRATION_SEGMENTS.json or legacy v2 segments"
    ),
    visual_events: Optional[str] = typer.Option(
        None, "--visual-events", help="Existing VISUAL_EVENT_INDEX.json to reuse"
    ),
    timeline: Optional[str] = typer.Option(
        None, "--timeline", help="Local timeline/readback JSON; omit until ChatCut readback exists"
    ),
    output_dir: Optional[str] = typer.Option(
        None, "--output-dir", "-o", help="Artifact directory (defaults to scene database folder)"
    ),
):
    """Build VISUAL_EVENT_INDEX, NARRATION_SEGMENTS, SYNC_MAP, and QC artifacts."""
    ...
    scene_path = Path(scene_database)
    target_dir = Path(output_dir) if output_dir else scene_path.parent
    target_dir.mkdir(parents=True, exist_ok=True)
    try:
        event_index = (
            load_visual_event_index(visual_events)
            if visual_events
            else build_visual_event_index(
                scene_path, target_dir / "VISUAL_EVENT_INDEX.json"
            )
        )
        if visual_events:
            (target_dir / "VISUAL_EVENT_INDEX.json").write_text(
                event_index.model_dump_json(indent=2) + "\n", encoding="utf-8"
            )
        narration_doc = load_narration_segments(narration, visual_events=event_index)
        bound_narration_path = target_dir / "NARRATION_SEGMENTS.json"
        # Windows treats narration_segments.json and NARRATION_SEGMENTS.json
        # as the same path; never overwrite the legacy input in place.
        if bound_narration_path.resolve() == Path(narration).resolve():
            bound_narration_path = target_dir / "NARRATION_SEGMENTS.v3.json"
        write_narration_segments(narration_doc, bound_narration_path)
        timeline_payload = (
            json.loads(Path(timeline).read_text(encoding="utf-8")) if timeline else None
        )
        sync_doc = build_sync_map(
            narration_doc,
            event_index,
            timeline=timeline_payload,
        )
        write_sync_map(sync_doc, target_dir / "SYNC_MAP.json")
        alignment_report = evaluate_alignment(sync_doc)
        (target_dir / "ALIGNMENT_REPORT.json").write_text(
            alignment_report.model_dump_json(indent=2) + "\n", encoding="utf-8"
        )
    except (OSError, ValueError, TypeError, json.JSONDecodeError) as exc:
        typer.echo(f"sync build failed: {exc}", err=True)
        raise typer.Exit(code=1)

    typer.echo(f"VISUAL_EVENT_INDEX = {target_dir / 'VISUAL_EVENT_INDEX.json'}")
    typer.echo(f"NARRATION_SEGMENTS = {bound_narration_path}")
    typer.echo(f"SYNC_MAP = {target_dir / 'SYNC_MAP.json'}")
    typer.echo(f"ALIGNMENT_STATUS = {alignment_report.status}")
    if alignment_report.status != "PASS":
        typer.echo("FINAL_READY = NO")
        raise typer.Exit(code=2)
    typer.echo("FINAL_READY = YES")


@app.command("edit")
def edit(
    backend: str = typer.Option("chatcut", "--backend", help="chatcut or ffmpeg"),
    sync_map: str = typer.Option(..., "--sync-map", help="SYNC_MAP.json"),
    project_id: Optional[str] = typer.Option(
        None, "--project-id", help="Exact ChatCut project ID from MCP/project readback"
    ),
    timeline_id: Optional[str] = typer.Option(
        None, "--timeline-id", help="Exact ChatCut timeline ID from project readback"
    ),
    dry_run: bool = typer.Option(False, "--dry-run", help="Only print the track/repair plan"),
):
    """Apply a synchronization plan through the selected editing backend."""

    from .alignment import load_sync_map
    from .alignment.qc import evaluate_alignment
    from .alignment.repair import plan_repairs
    from .integrations.chatcut import build_timeline_plan
    from .integrations.editing_backend import resolve_editing_backend

    try:
        sync_document = load_sync_map(sync_map)
        report = evaluate_alignment(sync_document)
        repair_plan = plan_repairs(report, sync_document)
        timeline_plan = build_timeline_plan(sync_document)
    except (OSError, ValueError, TypeError) as exc:
        typer.echo(f"edit failed: {exc}", err=True)
        raise typer.Exit(code=1)

    typer.echo(f"EDITING_BACKEND = {backend}")
    typer.echo(f"SYNC_QC = {report.status}")
    typer.echo(f"TRACK_PLAN_ITEMS = {len(timeline_plan.items)}")
    typer.echo(f"REPAIR_OPERATIONS = {len(repair_plan)}")
    if dry_run:
        for operation in repair_plan:
            typer.echo(
                f"  {operation.sync_id}: {operation.action} ({operation.reason})"
            )
        return
    if backend.lower() != "chatcut":
        # The existing FFmpeg path remains available to its owning pipeline;
        # this command does not pretend it has live-project readback.
        try:
            resolve_editing_backend(backend)
        except (RuntimeError, ValueError) as exc:
            typer.echo(str(exc), err=True)
            raise typer.Exit(code=2)
        typer.echo("FFmpeg remains the render/debug fallback; no live timeline was mutated.")
        return

    from .integrations.chatcut.client import probe_chatcut_connection

    connection = probe_chatcut_connection()
    typer.echo(connection.as_text())
    if not connection.ready_for_edit or not project_id:
        typer.echo(
            "CHATCUT_EDIT = BLOCKED; authenticate the official chatcut MCP and provide "
            "the exact project_id before any mutation.",
            err=True,
        )
        raise typer.Exit(code=2)
    # The current Codex session exposes no Python tool gateway.  Keep this
    # explicit rather than making an unaudited HTTP/database fallback.
    typer.echo(
        "CHATCUT_EDIT = BLOCKED; host MCP tool gateway is not exposed to this CLI session.",
        err=True,
    )
    raise typer.Exit(code=2)
